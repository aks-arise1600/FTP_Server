...
QString FtpControlConnection::toLocalPath(const QString &fileName) const
{
    ...
    qDebug().noquote() <<QDateTime::currentDateTime().toString("yyyy/MM/dd hh:mm:ss")<<__FUNCTION__<<":: "<< "to local path" << fileName << "->" << localPath;
    return localPath;
}

void FtpControlConnection::reply(const QString &replyCode)
{
    qDebug().noquote() <<QDateTime::currentDateTime().toString("yyyy/MM/dd hh:mm:ss")<<__FUNCTION__<<":: "<< "reply" << replyCode;
    socket->write((replyCode + "\r\n").toUtf8());
}

void FtpControlConnection::processCommand(const QString &entireCommand)
{
    qDebug().noquote() <<QDateTime::currentDateTime().toString("yyyy/MM/dd hh:mm:ss")<<__FUNCTION__<<":: "<< "command" << entireCommand;
